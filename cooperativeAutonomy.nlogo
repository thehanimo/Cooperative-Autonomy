...
to spawn-vehicles
  if not any? turtles with [ xcor <= ((- world-width + 3) / 2) ][ ;make sure there are no turtles in the first column of the world.
...
to go
  ...
  ask turtles with [xcor > ((world-width - 4) / 2)][die]  ;kill the turtles that reached the end of the world
  tick
end
